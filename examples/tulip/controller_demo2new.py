...
sys_safe |= {('((stage = 0) && (! (loc = 39))) -> X (stage = 0)')}
sys_safe |= {('((stage = 0) && (loc = 39)) -> X (stage = 1)')}
sys_safe |= {('((stage = 1) && (! (loc = 45))) -> X (stage = 1)')}
sys_safe |= {('((stage = 1) &&  (loc = 45)) -> X (stage = 2)')}
sys_safe |= {('((stage = 2) &&  !(loc = 16)) -> X (stage = 2)')}
for iter1 in range(dim):
        sys_safe |= {('!(loc = '+str(iter1+(dim))+')')}
...
dumpsmach.write_python_case("democontroller2new.py", ctrl, classname="ExampleCtrl2")
